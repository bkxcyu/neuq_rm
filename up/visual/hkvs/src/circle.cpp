        ...
        if (nRet == MV_OK)
        {
            ...
            //scanf("%d", &nInput);
                    ...
                    fwrite(pDataForRGB, 1, stConvertParam.nDstLen, fp);
                     ...
                     IplImage tmp_image= IplImage(src);
                     IplImage* pBinary = &tmp_image; 
                     
                     //深拷贝只要再加一次复制数据：  
                     IplImage *pImage = cvCloneImage(pBinary);  
                     IplImage *pImg8u=NULL; 
                     cvSmooth(pImg8u,pImg8u,CV_GAUSSIAN,5,5,0,0); //平滑
                     CvMemStorage* storage=cvCreateMemStorage(0);//声明storage变量，用于存储检测到的线段
                     CvSeq* circles=cvHoughCircles(pImg8u,storage,CV_HOUGH_GRADIENT,2,pImg8u->height/2,200,100,50,150);
                     int i=0;
