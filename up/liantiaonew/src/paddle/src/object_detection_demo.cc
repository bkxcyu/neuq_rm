...
public:
    ...
    std::string label_path = "/home/nvidia/RM/src/paddle/src/labels/pascalvoc_label_list";
    std::string model_path = "/home/nvidia/RM/src/paddle/src/models/model.nb";
...
std::vector<std::string> ImageConverter::load_labels(const std::string &path) {
  std::ifstream file;
...
void ImageConverter::preprocess(cv::Mat &input_image, const std::vector<float> &input_mean,
                const std::vector<float> &input_std, int input_width,
                int input_height, float *input_data) {
  cv::Mat resize_image;
...
std::vector<RESULT> ImageConverter::postprocess(const float *output_data, int64_t output_size,
                                const std::vector<std::string> &word_labels,
                                const float score_threshold,
                                cv::Mat &output_image, double time) {
...
cv::Mat ImageConverter::process(cv::Mat &input_image,
                std::vector<std::string> &word_labels,
                std::shared_ptr<paddle::lite_api::PaddlePredictor> &predictor) {
